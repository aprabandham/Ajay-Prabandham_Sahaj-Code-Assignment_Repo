...
from src.common import NUM_PLAYERS, round_game_type, round_player_index, game_claim_player_index, possible_claiming_player_types, round_game_name, possible_player_claim_labels

class TambolaFastestThirdRowGameSimulator(TambolaGameSimulatorInterface):
    ...
    def validate_technically_inaccurate_claim(self):
        self.log_message_to_file(f"Tambola Game Simulator {possible_player_claim_labels[possible_claiming_player_types["TECHNICALLY_INACCURATE"]]} Claim Validation - for Game {self.game_type} - {round_game_name[self.game_type]}")
        ...
        for player in player_list:
            if player.player_index == game_claim_player_index["FASTEST_THIRD_ROW"]:
                ...
                self.log_message_to_file(f"Claiming Player Index for Game = {game_claim_player_index["FASTEST_THIRD_ROW"]}")
                self.log_message_to_file(player)
                self.log_message_to_file(ticket)
    ...
    def validate_technically_accurate_but_invalid_claim(self):
        self.log_message_to_file(f"Tambola Game Simulator {possible_player_claim_labels[possible_claiming_player_types["TECHNICALLY_ACCURATE_BUT_INVALID"]]} Claim Validation - for Game {self.game_type} - {round_game_name[self.game_type]}")
        player_list = self._player_gen.generate_player_list()
        ticket_list = self.ticket_gen.generate_ticket_list()
    ...
    def validate_technically_accurate_and_valid_claim(self):
        self.log_message_to_file(f"Tambola Game Simulator {possible_player_claim_labels[possible_claiming_player_types["TECHNICALLY_ACCURATE_AND_VALID"]]} Claim Validation - for Game {self.game_type} - {round_game_name[self.game_type]}")
        player_list = self._player_gen.generate_player_list()
        ticket_list = self.ticket_gen.generate_ticket_list()
