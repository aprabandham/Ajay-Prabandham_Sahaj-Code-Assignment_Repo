...
def test_init_fastest_early_five_ticket_generator_call_seq_len_lt_lower_limit():
    call_seq = [3, 87, 22, 42]
...
def test_init_fastest_early_five_ticket_generator_call_seq_len_gt_upper_limit():
    call_seq = [i for i in range(1, 100)]
    
    with pytest.raises(ValueError):
        ticket_gen = FastestEarlyFiveTicketGenerator(call_seq, num_players, game_type, claim_player_index, claiming_player_type, claiming_player_sub_type)

def test_init_fastest_early_five_ticket_generator_num_players_lt_lower_limit_not_allowed():
    num_players = ZERO

    with pytest.raises(ValueError):
        ticket_gen = FastestEarlyFiveTicketGenerator(call_seq, num_players, game_type, claim_player_index, claiming_player_type, claiming_player_sub_type)

def test_init_fastest_early_five_ticket_generator_num_players_gt_upper_limit_not_allowed():
    num_players = 5

    with pytest.raises(ValueError):
        ticket_gen = FastestEarlyFiveTicketGenerator(call_seq, num_players, game_type, claim_player_index, claiming_player_type, claiming_player_sub_type)

def test_init_fastest_early_five_ticket_generator_game_type_lt_lower_limit_not_allowed():
    game_type = 0

    with pytest.raises(ValueError):
        ticket_gen = FastestEarlyFiveTicketGenerator(call_seq, num_players, game_type, claim_player_index, claiming_player_type, claiming_player_sub_type)

def test_init_fastest_early_five_ticket_generator_game_type_gt_upper_limit_not_allowed():
    game_type = 6

    with pytest.raises(ValueError):
        ticket_gen = FastestEarlyFiveTicketGenerator(call_seq, num_players, game_type, claim_player_index, claiming_player_type, claiming_player_sub_type)

def test_init_fastest_early_five_ticket_generator_claim_player_index_lt_lower_bound():
    claim_player_index = ZERO

    with pytest.raises(ValueError):
        ticket_gen = FastestEarlyFiveTicketGenerator(call_seq, num_players, game_type, claim_player_index, claiming_player_type, claiming_player_sub_type)

def test_init_fastest_early_five_ticket_generator_claim_player_index_gt_upper_bound():
    claim_player_index = num_players+1

    with pytest.raises(ValueError):
        ticket_gen = FastestEarlyFiveTicketGenerator(call_seq, num_players, game_type, claim_player_index, claiming_player_type, claiming_player_sub_type)

def test_fastest_early_five_ticket_generator_generate_ticket_list_ticket_list_not_empty():
    ticket_gen = FastestEarlyFiveTicketGenerator(call_seq, num_players, game_type, claim_player_index, claiming_player_type, claiming_player_sub_type)
    ticket_list = ticket_gen.generate_ticket_list()
    
    assert len(ticket_list) != 0

def test_fastest_early_five_ticket_generator_generate_ticket_list_num_tickets_exactly_matches_num_players():
    ticket_gen = FastestEarlyFiveTicketGenerator(call_seq, num_players, game_type, claim_player_index, claiming_player_type, claiming_player_sub_type)
    ticket_list = ticket_gen.generate_ticket_list()
    
    assert len(ticket_list) == num_players

def test_fastest_early_five_ticket_generator_generate_ticket_list_claiming_ticket_not_accurate_not_empty():
    game_type = round_game_type["FASTEST_EARLY_FIVE"]
    claim_player_index = game_claim_player_index["FASTEST_EARLY_FIVE"]
...
def test_fastest_early_five_ticket_generator_generate_ticket_list_claiming_ticket_accurate_but_invalid_claim_not_empty():
    game_type = round_game_type["FASTEST_EARLY_FIVE"]
    claim_player_index = game_claim_player_index["FASTEST_EARLY_FIVE"]
...
def test_fastest_early_five_ticket_generator_generate_ticket_list_claiming_ticket_accurate_and_valid_claim_not_empty():
    game_type = round_game_type["FASTEST_EARLY_FIVE"]
    claim_player_index = game_claim_player_index["FASTEST_EARLY_FIVE"]
    claiming_player_type = possible_claiming_player_types["TECHNICALLY_ACCURATE_AND_VALID"]
    claiming_player_sub_type = possible_claiming_player_technically_accurate_and_valid_sub_types["TECHNICALLY_ACCURATE_AND_VALID_TYPE_1"]
...
def test_fastest_early_five_ticket_generator_generate_ticket_list_claiming_ticket_inaccurate_and_non_claiming_tickets_not_empty():
    game_type = round_game_type["FASTEST_EARLY_FIVE"]
    claim_player_index = game_claim_player_index["FASTEST_EARLY_FIVE"]
    claiming_player_type = possible_claiming_player_types["TECHNICALLY_INACCURATE"]
    claiming_player_sub_type = possible_claiming_player_technically_inaccurate_sub_types["TECHNICALLY_INACCURATE_TYPE_1"]
...
def test_fastest_early_five_ticket_generator_generate_ticket_list_claiming_ticket_accurate_but_invalid_and_non_claiming_tickets_not_empty():
    game_type = round_game_type["FASTEST_EARLY_FIVE"]
    claim_player_index = game_claim_player_index["FASTEST_EARLY_FIVE"]
    claiming_player_type = possible_claiming_player_types["TECHNICALLY_ACCURATE_BUT_INVALID"]
    claiming_player_sub_type = possible_claiming_player_technically_accurate_but_invalid_sub_types["TECHNICALLY_ACCURATE_BUT_INVALID_TYPE_2"]
...
def test_fastest_early_five_ticket_generator_generate_ticket_list_claiming_ticket_accurate_and_valid_and_non_claiming_tickets_not_empty():
    game_type = round_game_type["FASTEST_EARLY_FIVE"]
    claim_player_index = game_claim_player_index["FASTEST_EARLY_FIVE"]
    claiming_player_type = possible_claiming_player_types["TECHNICALLY_ACCURATE_AND_VALID"]
    claiming_player_sub_type = possible_claiming_player_technically_accurate_and_valid_sub_types["TECHNICALLY_ACCURATE_AND_VALID_TYPE_3"]
